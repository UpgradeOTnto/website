...
echo "<div class='title'>Shop " .$config['server_name']. "</div>";
...
$categories = array(1 => 'Modificações de conta', 2 => 'Itens', 3 => 'Mochilas com itens', 4 => 'Addon itens');
if ($_SESSION['logged'] == 1) {
	if ($SHOP->isInstalled()) {
	if ($action == '') {
		if($ide->isAdmin()) {
					echo "<div class='toolbar' align='center'>";
					echo "<a href='".WEBSITE."/index.php/p/v/gifts/admin'>Shop Admin Painel</a>";
					echo "</div>";
		}
		alert("<b>Lembre-se:</b> Todas as categorias, <b>exceto para 'Mochilas com itens'</b>: Necessita ter espaço em sua <b>BACKPACK</b> para recebe-los.");
						...
						echo '<table class="table table-striped" border="0" cellspacing="1" cellpadding="4" width="100%">
						 ...
						 foreach($SQL->query('SELECT * FROM shop_offer WHERE category = '.$key.'') as $item) {
							...
							$image = 'public/images/items/'.$item['item'].'.gif';
								...
								<td>'.$item['points'].'</td>
								<td><button class="btn btn-info" type="submit" onClick="window.location.href=\''.WEBSITE.'/index.php/p/v/gifts/buy/'.$item['id'].'\';" class="ide_button">Comprar</button></td>
								</tr>';
						 }
						 echo '</table></div>';
					}
					else {
						alert("Não há produtos nesta categoria.");
						echo '</div>';
					}
			}
		echo '</div>';
	...
	elseif ($action == 'buy') {
		...
		if (!$product) {
			...
			echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts">Voltar ao shop</a></div>';
		}
		else {
					...
					if($info['type'] != '4') {
						if($_POST['other_char']) {
							if($SQL->query('SELECT * FROM players WHERE name = "'.$_POST['other_char'].'"')->fetch()) {
								...
								echo '<div align="right"><a href="'.WEBSITE.'/index.php/p/v/gifts/buy/'.$info['id'].'">Voltar ao Shop</a></div>';
								$send = false;
							}
						}
						else {
