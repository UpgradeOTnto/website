...
class Account_model extends CI_Model {
	...
	function check_login() {
		...
		$this->db->where('accounts', array('name' => $_POST['name'], 'password' => sha1($_POST['pass'])));
		//$row = $this->db->get()->row();
		...
		//$sql = array('name ' => $_POST['name'], 'password' => sha1($_POST['pass']));		
		//$this->db->where($sql);
		// $SQL = 'select * from accounts where name = \'' .$_POST['name'] . '\' and password = \'' . sha1($_POST['pass']) . '\'' ;		
